...
//
// instruction 與 prompt 都用英文，避免 FM 模型被中文 framing 鎖住，
// 否則 zh→en 時模型會誤判輸入「已是英文」直接原樣回傳。

struct TranslateHandler: Sendable {

    // 語言代碼 → 英文語言名（給 prompt 用）
    ...
    // 目標語言寫的 instructions — 讓模型切到該語言的「保留外來品牌名」習慣，
    // 否則用英文 instructions 時模型會在 en→zh 套用過度在地化（Apple→蘋果）。
    ...
    func translate(
        ...
        if sourceLang == "自動偵測" || Self.langNames[sourceLang] == nil {
            sourcePart = ""
        } else {
